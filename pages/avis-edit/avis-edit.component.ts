...
export class AvisEditComponent implements OnInit{
  ...
  constructor(private router: ActivatedRoute, private restaurantService: RestaurantService, private themeService: ThemeService, private avisService: AvisService){}
...
ngOnInit(): void {
  ...
  this.themeService.readAll().subscribe((data: Theme[])=> this.themes = data);
  this.restaurantService.search('').subscribe((data: Restaurant[])=> this.restos = data);

  const idStr= this.router.snapshot.paramMap.get("id");
    this.id = idStr ? +idStr : 0;
    this.restaurantService.read(this.id).subscribe((data: Restaurant)=> this.resto = data);
}

public enregistrerAvis() {
  this.newAvis = this.avisForm.value;
  this.newAvis.restaurant = this.resto
  this.newAvis.theme = this.themes.find(th => th.id === Number(this.newAvis.theme))
  ...
  if (this.avisForm.status === "INVALID") {
    ...
    this.avisService.post(this.newAvis).subscribe((data:Avis)=>this.newAvis = data);
